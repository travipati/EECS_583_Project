...
opt -loop-simplify < simple.bc > simple.ls.bc || { echo "Failed to opt loop simplify"; exit 1; }
...
#clang -emit-llvm -o simple.bc -c simple.c || { echo "Failed to emit llvm bc"; exit 1; }
../../../../../opt/llvm-debug/Release+Debug+Asserts/bin/opt -load /home/alexcope/EECS_583_Project/pass/Release+Debug+Asserts/lib/proj.so -profile-loader -profile-info-file=llvmprof.out -HoistLoads -o modif.bc < simple.ls.bc > /dev/null || { echo "Failed to optload"; exit 1; }
...
opt -dot-cfg modif.bc
llc modif.bc -o modif.s
