...
#clang -emit-llvm -o simple.bc -c simple.c || { echo "Failed to emit llvm bc"; exit 1; }
../../../../../opt/llvm-debug/Release+Debug+Asserts/bin/opt -load /home/alexcope/EECS_583_Project/pass/Release+Debug+Asserts/lib/proj.so -debug -profile-loader -profile-info-file=llvmprof.out -loop-unroll -o modif.bc < simple.ls.bc > /dev/null || { echo "Failed to optload"; exit 1; }
